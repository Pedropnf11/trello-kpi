            ...
            <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
                ...
                ${state.error ? `
                    <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg mb-8 text-sm font-medium text-left shadow-sm">
                        <div class="flex items-center gap-2 mb-1 font-bold">
                            ...
                            ${(UI._lpLang || 'pt') === 'en' ? 'Connection Error' : 'Erro de Conexão'}
                        </div>
                        ${state.error}
                    </div>
                ` : ''}
                ...
                <div class="space-y-8 relative z-10">
                    <button id="loginTrelloBtn"
                        ...
                        <span>${(UI._lpLang || 'pt') === 'en' ? 'Sign in with Trello' : 'Entrar com Trello'}</span>
                    </button>

                    <p class="text-xs text-gray-400 max-w-xs mx-auto leading-relaxed px-4">
                        ${(UI._lpLang || 'pt') === 'en' ? 'You will be securely redirected to Trello to authorize access to your boards.' : 'Serás redirecionado de forma segura para o Trello para autorizar o acesso aos teus quadros.'}
                    </p>
                </div>
                ...
                <div class="mt-12 pt-6 border-t border-gray-100">
                    <button id="showManualConfig" class="text-xs font-semibold text-gray-400 hover:text-gray-600 transition flex items-center justify-center gap-1 mx-auto group">
                        <span>${(UI._lpLang || 'pt') === 'en' ? 'Manual Setup' : 'Configuração Manual'}</span>
        ...
        <div class="min-h-screen flex items-center justify-center bg-gray-50 p-6">
            <div class="bg-white rounded-3xl shadow-xl p-10 max-w-4xl w-full border border-gray-100 flex flex-col max-h-[90vh]">
                <div class="mb-8 flex justify-between items-end">
                    <div>
                        <h2 class="text-3xl font-bold text-gray-900 mb-2">${(UI._lpLang || 'pt') === 'en' ? 'Your Boards' : 'Os teus Quadros'}</h2>
                        <p class="text-gray-500">${(UI._lpLang || 'pt') === 'en' ? 'Select the board you want to analyze.' : 'Seleciona o quadro que queres analisar.'}</p>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="App.setRole(null)" class="text-sm text-gray-500 hover:text-gray-700 font-bold px-4 py-2 rounded-lg hover:bg-gray-100 transition border border-transparent">
                            ${(UI._lpLang || 'pt') === 'en' ? 'Switch Role' : 'Trocar Função'}
                        </button>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 overflow-y-auto pr-2 custom-scrollbar flex-1 pb-4">
                    ...
                    `).join('') : `
                        <div class="col-span-full py-12 text-center text-gray-400">
                            <p class="text-lg">${(UI._lpLang || 'pt') === 'en' ? 'No open boards found.' : 'Não encontrámos quadros abertos.'}</p>
                            <p class="text-sm mt-2">${(UI._lpLang || 'pt') === 'en' ? 'Check your permissions or if the boards are archived.' : 'Verifica se tens permissões ou se os quadros estão arquivados.'}</p>
                        </div>
                    `}
                </div>

                <div class="mt-6 pt-4 border-t border-gray-100 text-center text-xs text-gray-400 font-medium">
                    ${(UI._lpLang || 'pt') === 'en' ? `Showing ${boards.length} available boards` : `Mostrando ${boards.length} quadros disponíveis`}
                </div>
            </div>
        </div>
        `;
        ...
        <div class="min-h-screen flex items-center justify-center bg-gray-100">
            <div class="bg-white rounded-2xl shadow-xl p-8 max-w-lg w-full">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-bold text-white-900">${(UI._lpLang || 'pt') === 'en' ? 'Manual Setup' : 'Configuração Manual'}</h2>
                    <button id="backToLogin" class="text-sm text-gray-500 hover:text-gray-900 font-medium">${(UI._lpLang || 'pt') === 'en' ? 'Back' : 'Voltar'}</button>
                </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="apiKey" value="${state.apiKey}" readonly class="w-full p-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-500 cursor-not-allowed">
                            <p class="text-[10px] text-gray-400 mt-1">${(UI._lpLang || 'pt') === 'en' ? 'Application key (hardcoded)' : 'Chave da aplicação (hardcoded)'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">${(UI._lpLang || 'pt') === 'en' ? 'Webhook URL (Optional - For Email Export)' : 'Webhook URL (Opcional - Para Exportar Email)'}</label>
                    ...
                    <button id="conectarManualBtn" class="w-full bg-gray-900 text-white py-3 rounded-lg font-bold hover:bg-gray-800 mt-6 shadow-lg transition-transform transform hover:-translate-y-0.5">
                        ${(UI._lpLang || 'pt') === 'en' ? 'Connect Manually' : 'Conectar Manualmente'}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="min-h-screen flex items-center justify-center bg-gray-50 p-6">
            <div class="bg-white rounded-3xl shadow-xl p-10 max-w-2xl w-full border border-gray-100 flex flex-col items-center text-center">
                <div class="mb-10">
                    ...
                    <h2 class="text-3xl font-black text-gray-900 mb-4">${(UI._lpLang || 'pt') === 'en' ? 'Welcome!' : 'Bem-vindo!'}</h2>
                    <p class="text-gray-500 text-lg max-w-md mx-auto">${(UI._lpLang || 'pt') === 'en' ? 'To personalize your experience, tell us your role in the team.' : 'Para personalizar a tua experiência, diz-nos qual é a tua função na equipa.'}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-8">
                    ...
                    <button onclick="App.setRole('manager')" class="group relative flex flex-col items-center p-8 bg-white border-2 border-slate-100 rounded-[2rem] hover:border-blue-500 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 w-full text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 group-hover:text-blue-600 mb-2">${(UI._lpLang || 'pt') === 'en' ? 'I\'m a Manager' : 'Sou Gestor'}</h3>
                        <p class="text-sm text-gray-400 group-hover:text-gray-500">${(UI._lpLang || 'pt') === 'en' ? 'I want to see overall team performance and manage funnels.' : 'Quero ver a performance geral da equipa e gerir os funis.'}</p>
                        ...
                        <div class="mt-6 px-4 py-2 bg-gray-50 rounded-full text-xs font-bold text-gray-400 group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors">
                            ${(UI._lpLang || 'pt') === 'en' ? 'Full Access' : 'Acesso Completo'}
                        </div>
                    </button>
                    ...
                    <button onclick="App.setRole('sales')" class="group relative flex flex-col items-center p-8 bg-white border-2 border-slate-100 rounded-[2rem] hover:border-green-500 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 w-full text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 group-hover:text-green-600 mb-2">${(UI._lpLang || 'pt') === 'en' ? 'I\'m an Agent' : 'Sou Vendedor'}</h3>
                        <p class="text-sm text-gray-400 group-hover:text-gray-500">${(UI._lpLang || 'pt') === 'en' ? 'I want to focus on my numbers and my tasks.' : 'Quero focar-me nos meus números e nas minhas tarefas.'}</p>
        ...
        <div id="roleSelectionModal" class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">
            <div class="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full mx-4 transform transition-all scale-100 border border-white/20">
                <div class="text-center mb-8">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900">${(UI._lpLang || 'pt') === 'en' ? 'Who are you?' : 'Quem és tu?'}</h3>
                    <p class="text-gray-500 mt-2">${(UI._lpLang || 'pt') === 'en' ? 'Choose how you want to view the board' : 'Escolhe como queres visualizar o quadro'} <span class="font-semibold text-gray-700">"${Utils.escapeHtml(boardName)}"</span></p>
                </div>
                ...
                <div class="grid gap-4">
                    <button onclick="App.confirmRole('${boardId}', 'manager')" class="group relative flex items-center p-4 bg-white border-2 border-slate-100 rounded-2xl hover:border-blue-500 hover:shadow-lg transition-all text-left w-full">
                        ...
                        <div>
                            <div class="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">${(UI._lpLang || 'pt') === 'en' ? 'I\'m a Manager' : 'Sou Gestor'}</div>
                            <div class="text-xs text-gray-500">${(UI._lpLang || 'pt') === 'en' ? 'Full access to all metrics and team.' : 'Acesso total a todas as métricas e equipa.'}</div>
                        </div>
                    ...
                    <button onclick="App.confirmRole('${boardId}', 'sales')" class="group relative flex items-center p-4 bg-white border-2 border-slate-100 rounded-2xl hover:border-green-500 hover:shadow-lg transition-all text-left w-full">
                        ...
                        <div>
                            <div class="font-bold text-gray-900 group-hover:text-green-600 transition-colors">${(UI._lpLang || 'pt') === 'en' ? 'I\'m an Agent' : 'Sou Vendedor'}</div>
                            <div class="text-xs text-gray-500">${(UI._lpLang || 'pt') === 'en' ? 'Focus on my performance and tasks.' : 'Foco na minha performance e tarefas.'}</div>
                        </div>
                ...
                <div class="mt-8 text-center">
                    <button onclick="document.getElementById('roleSelectionModal').remove()" class="text-sm text-gray-400 hover:text-gray-600 font-medium transition-colors">${(UI._lpLang || 'pt') === 'en' ? 'Cancel' : 'Cancelar'}</button>
                </div>
            </div>
        </div >
        `;
