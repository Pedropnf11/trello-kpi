...
export default function Sidebar() {
    ...
    const handleChangeRole = () => { resetPipelineAndRole(); router.push('/'); };

    // Gráficos removido — só Dashboard e Produtividade para o manager
    const links = role === 'sales'
        ...
        : [{ name: 'Dashboard',     href: '/dashboard/manager',               icon: LayoutDashboard },
           { name: 'Produtividade', href: '/dashboard/manager/productivity',  icon: Activity        }];
    ...
    const isActive = (href: string) => {
        if (href === '/dashboard/manager/productivity') return pathname.includes('/productivity');
            ...
            >
                ...
                <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                    ...
                    <div className="px-4 pt-5 pb-3 mt-2 border-t" style={{ borderColor: 'var(--border)' }}>
                        <div className="flex items-center justify-between mb-3">
                            <p className="text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
                        ...
                        <div className="grid grid-cols-3 gap-1 mb-2">
                            {quickRanges.map(({ label, days }) => {
                                ...
                                return (
                                    <button key={label} onClick={() => { setStartDate(ss); setEndDate(es); }}
                        ...
                        {(['start', 'end'] as const).map(t => (
                            <div key={t} className="relative mb-1.5">
                                ...
                                />
                                <span className="absolute right-3 top-2 text-[9px] font-black uppercase pointer-events-none" style={{ color: 'var(--text-muted)' }}>
                                    {t === 'start' ? 'Início' : 'Fim'}
                                </span>
                            </div>
                        ))}
                ...
                <div className="p-3 border-t space-y-0.5" style={{ borderColor: 'var(--border)' }}>
                    <button onClick={handleChangeRole}
                        ...
                        <RefreshCw className="w-4 h-4 shrink-0" style={{ color: 'var(--text-muted)' }} />
                        Trocar função
                    </button>
                    ...
                    {confirmLogout ? (
                        <div className="px-3 py-2 rounded-lg border" style={{ background: 'rgba(239,68,68,0.05)', borderColor: 'rgba(239,68,68,0.15)' }}>
                            <p className="text-[10px] font-bold text-red-400 mb-2">Tens a certeza?</p>
                            <div className="flex gap-2">
                                <button onClick={handleLogout} className="flex-1 py-1.5 rounded-lg text-[10px] font-black text-white bg-red-600 hover:bg-red-500 transition-colors">Sair</button>
                                <button onClick={() => setConfirmLogout(false)} className="flex-1 py-1.5 rounded-lg text-[10px] font-black transition-colors"
                                    style={{ background: 'var(--bg-surface)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>Cancelar</button>
                            </div>
                        </div>
                    ) : (
                        <button onClick={() => setConfirmLogout(true)}
                            ...
                            style={{ color: 'var(--text-secondary)' }}>
                            <LogOut className="w-4 h-4 shrink-0 group-hover:text-red-400" style={{ color: 'var(--text-muted)' }} />
